...
import { mangroveCardsData, mangroveConfig } from './data/mangroveData';
...
const MangroveSections = () => {
    ...
    const getCardStyle = (cardId) => {
        ...
        return {
            ...
            backgroundColor: isExpanded ? mangroveConfig.expandedBackgroundColor : mangroveConfig.backgroundColor,
            border: isExpanded ? `2px solid ${mangroveConfig.borderColor}` : '2px solid transparent'
        };
    };
  return (
    <div className="px-16 py-16 bg-white">
      ...
      {/* Title */}
      <h2 className="font-bold text-4xl text-center mb-16" style={{ color: mangroveConfig.primaryColor }}>
        {mangroveConfig.title}
      </h2>
        ...
        {mangroveCardsData.map((card) => (
            (!Object.values(expandedCards).some(expanded => expanded) || expandedCards[card.id]) && (
                ...
                >
                    <div className="absolute top-6 right-6">
                        {expandedCards[card.id] ? (
                            <ShrinkIcon 
                                ...
                                color={mangroveConfig.primaryColor} 
                            />
                        ) : (
                            <ExpandIcon 
                                onClick={() => handleToggleCard(card.id)} 
                    ...
                    <h3 className="font-bold text-2xl mb-4" style={{ color: mangroveConfig.primaryColor }}>
                        {card.title}
                    </h3>
                    <p className={`text-gray-600 leading-relaxed ${card.hasButton ? 'mb-6' : ''}`}>
                    ...
                    {card.hasButton && (
                        <button
                            ...
                            style={{ backgroundColor: mangroveConfig.primaryColor }}
                        >
...
export default MangroveSections